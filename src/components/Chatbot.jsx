...
export default function Chatbot() {
  ...
  const [messages, setMessages] = useState([
    {
      role: "bot",
      text: "Hi 👋 I'm your Delivery AI assistant!",
      options: ["Check delivery risk", "Optimize route", "Why delays happen?"]
    }
  ]);
  ...
  // 🔥 SEND FUNCTION (supports buttons too)
  const send = async (customText) => {
    const messageText = customText || input;
    if (!messageText.trim()) return;
  
    setMessages(p => [...p, { role: "user", text: messageText }]);
    setInput("");
    setLoading(true);
  
    try {
      const r = await axios.post("http://127.0.0.1:8001/chat", {
        message: messageText
      });
      ...
      setMessages(p => [
        ...p,
        {
          ...
          options: ["Check delivery risk", "Optimize route"]
        }
      ]);
    }
        ...
        {open && (
          ...
          >
            ...
            <div className="p-3 flex items-center gap-2" style={{ borderBottom: "1px solid var(--border)" }}>
              <div className="w-7 h-7 rounded-lg text-white text-xs flex items-center justify-center"
                style={{ background: "var(--accent)" }}>
                🤖
              </div>
              <div>
                ...
                <span className="text-[9px]" style={{ color: "var(--risk-low)" }}>
                  ● Online
                </span>
              </div>
            </div>
            ...
            <div className="flex-1 p-3 space-y-3 overflow-y-auto">
              ...
              {messages.map((m, i) => (
                <div key={i}>
                  ...
                  <div className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                    <div
                      className="max-w-[80%] text-[11px] px-3 py-2 rounded-xl"
                      style={{
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      {m.options.map((opt, idx) => (
                        <button
                          key={idx}
                          onClick={() => send(opt)}
                          className="text-[10px] px-3 py-1 rounded-lg border hover:opacity-80"
                          style={{
              ...
              {/* LOADING */}
              {loading && (
                <div className="flex gap-1 px-3 py-2 rounded-xl w-fit"
